...
def create_access_token(data: dict) -> str:
    ...
    to_encode.update({"exp": expire, "user_id": data.get("user_id", 1)})  # Include user_id for auditing
...
async def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    return {"user_id": user.id, "username": username, "role": role, "business_ids": business_ids}
