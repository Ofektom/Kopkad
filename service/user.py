...
async def signup_unauthenticated(request: SignupRequest, db: Session):
    ...
    if input_phone.startswith("234") and len(input_phone) == 13:
        ...
        return error_response(status_code=400, message="Phone number must be 10 digits (with or without leading 0) or include country code 234")

    valid_roles = {Role.SUPER_ADMIN, Role.ADMIN, Role.AGENT, Role.SUB_AGENT, Role.CUSTOMER}
    ...
    if requested_role not in {Role.CUSTOMER, Role.AGENT}:
        return error_response(status_code=403, message="Only CUSTOMER and AGENT can sign up without authentication")
    ...
    if db.query(User).filter(User.phone_number == phone_number).first():
        return error_response(status_code=status.HTTP_409_CONFLICT, message="Phone number already in use")
    if db.query(User).filter(User.username == phone_number).first():
        return error_response(status_code=status.HTTP_409_CONFLICT, message="Username (derived from phone number) already in use")
    ...
    if requested_role == Role.AGENT and request.business_code:
        return error_response(status_code=400, message="AGENT cannot sign up with a business_code; use /business/create to create a business after signup")

    business = None
    if requested_role == Role.CUSTOMER:
        if request.business_code:
            business = db.query(Business).filter(Business.unique_code == request.business_code).first()
            if not business:
            ...
            if not business:
                return error_response(status_code=500, message="Default business CEN123 not found in database")
    ...
    try:
        ...
        if requested_role == Role.CUSTOMER:
            db.execute(insert(user_business).values(user_id=user.id, business_id=business.id))
            db.commit()

        if requested_role == Role.AGENT:
            db.execute(insert(user_permissions).values([{"user_id": user.id, "permission": Permission.CREATE_SUB_AGENT}]))
            db.commit()

        settings_obj = Settings(user_id=user.id, notification_method=NotificationMethod.BOTH)
        db.add(settings_obj)
        db.commit()

        # Send welcome email and handle failure
        if request.email and settings_obj.notification_method in ["email", "both"]:
            email_result = await send_welcome_email(request.email, user.full_name, phone_number, requested_role)
            if email_result["status"] == "error":
                logger.error(email_result["message"])
                # Optionally rollback if email is critical
                # db.rollback()
                # return error_response(status_code=500, message=email_result["message"])
            else:
...
async def signup_authenticated(request: SignupRequest, db: Session, current_user: dict):
    ...
    if current_user_role not in {Role.SUPER_ADMIN, Role.ADMIN, Role.AGENT}:
        return error_response(status_code=403, message="Insufficient permissions to create a user")
    if requested_role != Role.SUB_AGENT and current_user_role not in {Role.ADMIN, Role.SUPER_ADMIN}:
        return error_response(status_code=403, message="Only ADMIN or SUPER_ADMIN can create non-SUB_AGENT roles")
    ...
    if current_user_role == Role.AGENT:
        ...
        if not business:
            return error_response(status_code=403, message="Agent must register a business before creating sub-agents")
    ...
    try:
        ...
        if request.email and (settings_obj.notification_method in ["email", "both"]):
            await send_welcome_email(request.email, user.full_name, phone_number, requested_role)
...
async def login(request: LoginRequest, db: Session):
    ...
    if input_username.startswith("234") and len(input_username) == 13:
        ...
        return error_response(status_code=400, message="Username must be a valid phone number format (e.g., +2348000000003, 2348000000003, 8000000003)")
    ...
    business_ids = [row[0] for row in db.query(user_business.c.business_id).filter(user_business.c.user_id == user.id).all()]
    if not business_ids:
        return error_response(status_code=403, message="User must be associated with at least one business")
...
async def handle_oauth_callback(provider: str, code: str, state: str, db: Session):
    ...
    async with httpx.AsyncClient() as client:
        if provider == "google":
            logger.info(f"Processing Google OAuth callback with code: {code}, state: {state}")
            ...
            if "error" in token_data:
                return error_response(status_code=400, message=f"Failed to authenticate with Google: {token_data.get('error_description', 'No details provided')}")
            ...
            token_data = token_response.json()
            if "error" in token_data:
                return error_response(status_code=400, message="Failed to authenticate with Facebook")
            user_info_response = await client.get(
...
async def get_refresh_token(refresh_token: str):
    ...
    if not refresh_data:
        return error_response(status_code=401, message="Invalid or expired refresh token")
    return success_response(
