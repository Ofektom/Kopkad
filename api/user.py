...
from service.user import signup_unauthenticated, signup_authenticated, login, handle_oauth_callback, get_refresh_token
...
async def signup_unauthenticated_endpoint(request: SignupRequest, db: Session = Depends(get_db)):
    return await signup_unauthenticated(request, db)

@user_router.post("/signup-authenticated", response_model=Response)
async def signup_authenticated_endpoint(request: SignupRequest, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
