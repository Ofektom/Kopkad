...
async def whatsapp_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    expected_signature = (
        "sha256="
        + hmac.new(
            settings.WHATSAPP_VERIFY_TOKEN.encode(), body, hashlib.sha256
        ).hexdigest()
    )
    ...
    if "messages" not in data.get("entry", [{}])[0].get("changes", [{}])[0].get(
        "value", {}
    ):
    ...
    pending_request = (
        ...
        .filter(PendingBusinessRequest.customer_id == customer.id)
        .order_by(PendingBusinessRequest.created_at.desc())
        .first()
    )

    if not pending_request or pending_request.expires_at < datetime.now(timezone.utc):
        await send_whatsapp_notification(
            from_number, "No active invitation found or it has expired."
        )
        return {"status": "error", "message": "No active invitation"}

    business = (
        db.query(Business).filter(Business.id == pending_request.business_id).first()
    )
    try:
        if body == "accept":
            db.execute(
            ...
            db.commit()
            await send_whatsapp_notification(
                from_number, f"You have successfully joined {business.name}!"
            )
        elif body == "reject":
            db.delete(pending_request)
            db.commit()
            await send_whatsapp_notification(
                from_number,
                f"You have rejected the invitation to join {business.name}.",
            )
        else:
            await send_whatsapp_notification(
                from_number, "Please reply with 'Accept' or 'Reject'."
            )
