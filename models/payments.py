...
class PaymentRequest(Base, AuditMixin):
    ...
    account_details_id = Column(Integer, ForeignKey("account_details.id"), nullable=False)
    savings_account_id = Column(Integer, ForeignKey("savings_accounts.id"), nullable=False)
    amount = Column(Numeric(10, 2), nullable=False)
    status = Column(Enum(PaymentRequestStatus), nullable=False, default=PaymentRequestStatus.PENDING)
